...
class ManimScene(BaseModel):
    ...
    audio_duration: Optional[float] = Field(
        default=None,
        description="The exactly calculated duration of the TTS audio in seconds. Set by the backend."
    )
    audio_path: Optional[str] = Field(
        default=None,
        description="The absolute path to the generated TTS audio file. Set by the backend."
    )
...
class DirectorScene(BaseModel):
    """A high-level plan for a single scene generated by the Director Agent."""
    scene_id: str = Field(..., description="e.g. 'scene_1'")
    narration_text: str = Field(..., description="The spoken narration script. Avoid markdown.")
    visual_description: str = Field(..., description="A plain english description of what exactly should be drawn and animated on screen to match the narration.")
    estimated_duration: float = Field(..., description="Roughly how many seconds this will take.")

class DirectorStoryboard(BaseModel):
    """The complete multi-minute plan generated by the Director Agent."""
...
class ManimDraftResponse(BaseModel):
    ...
    draft: DirectorStoryboard
    message: str = "Director plan generated. POST to /api/v1/render to render."
